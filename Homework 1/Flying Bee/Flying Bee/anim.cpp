 ...
 **********************************************************
 ...
 DOES: These functions draw the components of the world. Some are 
 helper functions of helper functions (e.g. drawWing()).
 
 **********************************************************
 **********************************************************
 **********************************************************/

void drawGround(float ground_level)
{
...
void drawTree(float ground_level)
{
    // push old model_view
    mvstack.push(model_view);
    
    float shear_amount = sinf((float) TIME ) / 5.0f;
    
    // Brown
    set_colour(0.59f, 0.3f, 0.0f);
    
    // Segment 1
    model_view *= Scale(0.3f, 1.0f, 0.3f);
    model_view *= Translate(shear_amount / 2.0f, ground_level + 0.55f, 0.0f);
    model_view *= ShearX(shear_amount);
    drawCube();
    
    // Segments 2 - 8
    for (int i = 1; i < 8; i++)
    {
        model_view *= Scale(1.0f, 1.0f - fabs(shear_amount) * 0.1f, 1.0f);
        model_view *= Translate(shear_amount / 2.0f, 1.0f, 0.0f);
        model_view *= ShearX(shear_amount);
        drawCube();
    }
    ...
    // Draw the foliage
    // Red
    set_colour(1.0f, 0.0f, 0.5f);
    // undo shearing effects
    model_view *= Scale(1.5f / 0.3f, 1.5f / ((1.0f - (fabs(shear_amount) * 0.1f) * 8 )) , 1.5f / 0.3f);
    model_view *= Translate(shear_amount / 2.0f, 1.0f - fabs(shear_amount) * 0.1f, 0.0f);
    model_view *= ShearX(-shear_amount * 3);
    drawSphere();
...
void drawWings(float bl, float bw, float bh, float theta_r, float theta_d)
{
    mvstack.push(model_view);
    ...
    set_colour(0.5f, 0.5f, 0.5f); // grey
    
    // Left Wing
    mvstack.push(model_view);
    model_view *= Translate(0.0f, bh / 2.0f, bw / 2.0f + w_width / 2.0f);
    model_view *= Translate(0.0f, -(sinf(theta_d)) * w_width / 2.0f, (cosf(theta_d) - 1.0f) * w_width / 2.0f);
    model_view *= RotateX(theta_r);
    model_view *= Scale(w_length, w_height, w_width);
    drawCube();
    model_view = mvstack.pop();
...
void drawLegs(float bl, float bw, float bh, float r_factor)
{
    mvstack.push(model_view);
    ...
    float   l_length    = 0.2f,
            l_width     = 0.2f,
            l_height    = 0.8f;
    float   theta_r     = sinf(TIME * 2.0f) * r_factor;
    float   theta_d     = theta_r * (float)PI / 180.0f;
    float   theta_d2    = (theta_r + r_factor) * (float)PI / 180.0f;
    float   sub_angle   = theta_d / 2.0f;
    
    set_colour(0.5f, 0.5f, 0.5f); // grey
    
    // Right legs
    for (float f = -0.5; f < 1.0f; f += 0.5f)
    {
        mvstack.push(model_view);
        model_view *= Translate(f, -(bh / 2.0f + l_height / 2.0f), bw / 2.0f);
        model_view *= Translate(0.0f, -((cosf(sub_angle) - 1.0) * l_height / 2.0f), -(sinf(sub_angle)) * l_height / 2.0f);
        model_view *= RotateX(theta_r / 3.0f);
        model_view *= Scale(l_length, l_height, l_width);
        drawCube();
        
        model_view *= Scale(1.0f / l_length, 1.0f / l_height, 1.0f / l_width);
        model_view *= Translate(0.0f, -l_height, 0.0f);
        model_view *= Translate(0.0f, sinf(theta_d2) * l_width / 2.0f, -(sinf(theta_d2/3.0f)) * l_height / 2.0f);
        model_view *= RotateX((theta_r + r_factor) / 3.0f);
        model_view *= Scale(l_length, l_height, l_width);
        drawCube();
...
void drawWasp(void)
{
    ...
    // Rendundant push / pop at beginning and end of function to be safe
    mvstack.push(model_view);
    ...
    float   h_length    = 0.5f,
            h_width     = 0.5f,
            h_height    = 0.5f;
    float   r_factor    = 50.0f;
    float   theta_r     = sinf(TIME * 2.0f) * r_factor;
    float   theta_d     = theta_r * (float)PI / 180.0f;
    
    // Oscilate the wasp up and down
    model_view *= Translate(0.0f, theta_r / 19.f, 0.0f);
    
    // Draw torso
    mvstack.push(model_view);
    set_colour(0.5f, 0.5f, 0.5f); // grey
    model_view *= Scale(t_length, t_height, t_width);
    drawCube();
    model_view = mvstack.pop();
    
    // Draw butt
    mvstack.push(model_view);
    set_colour(0.68f, 0.8f, .18); // yellow
    model_view *= Translate(t_length / 2.0f + b_length, 0.0f, 0.0f);
    model_view *= Scale(b_length, b_height, b_width);
    drawSphere();
    model_view = mvstack.pop();
    
    // Draw head
    mvstack.push(model_view);
    set_colour(0.5f, 0.0f, 0.5f); // purple
    model_view *= Translate(-(t_length / 2.0f + h_length), 0.0f, 0.0f);
    model_view *= Scale(h_length, h_height, h_width);
    drawSphere();
    model_view = mvstack.pop();
    
    // Draw wings
    drawWings(t_length, t_width, t_height, theta_r, theta_d);
    
    // Draw legs
    drawLegs(t_length, t_width, t_height, r_factor);
...
void display(void)
{
    ...
    /*
    ** Starting with a clean model_view matrix,
    ** otherwise these functions won't work as intended
    */
    float ground_level = -5.0f;
    
    drawGround(ground_level);
    drawTree(ground_level);
    
//    mvstack.push(model_view);
//    model_view *= RotateY(-TIME * 20);
