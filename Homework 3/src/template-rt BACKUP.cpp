...
float intersect(Ray ray, Sphere sphere)
{
    ...
    mat4 M = getSphereMatrix(sphere);
    mat4 M_inverse;
    bool invert_success = InvertMatrix(M, M_inverse);
    if (invert_success == false)
...
vec4 trace(const Ray& ray)
{
    ...
    if (lowest_index >= 0)
    {
        ...
        return vec4(temp.c_r * amb_r, temp.c_g * amb_g, temp.c_b * amb_b, 1.0f);
    }
    ...
    for (int y = 0; y < g_height; y++)
        for (int x = 0; x < g_width; x++)
            for (int i = 0; i < 3; i++)
            {
                ...
                if (c > 1)
                    c = 1;
                buf[y*g_width*3+x*3+i] = (unsigned char)(c * 255.9f);
            }
